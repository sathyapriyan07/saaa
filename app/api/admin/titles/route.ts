...
import { createTitle } from '../../../services/titles.service';
import { TitleSchema } from '../../../validation/title.schema';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parse = TitleSchema.safeParse(body);
    if (!parse.success) {
      return NextResponse.json({ success: false, message: 'Validation error', errors: parse.error.errors }, { status: 400 });
    }
    const data = await createTitle(parse.data);
    return NextResponse.json({ success: true, data });
  } catch (error: any) {
    console.error('POST /api/admin/titles error:', error);
